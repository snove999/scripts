...
const CREATOR_MEDIA = {
  "cmrlsinpo361z07o5a9bpvuv4": "cmrkir1jv18d107o6r66qpqjl"
};
const media = (id, dramaId="") => {
  const creatorId = CREATOR_MEDIA[dramaId];
  return creatorId
    ? `${CDN}/uploads/creator/${creatorId}/${id}/video.mp4`
    : `${CDN}/uploads/direct/${id}/video.mp4`;
};

// Adult/secret candidates already exposed by public theater/feed APIs.
const SECRET_ITEMS = [
...
].map(x => ({...x, price:0, unlocked:true}));

function secretList() {
  return JSON.stringify([{result:{data:{json:{items:SECRET_ITEMS,nextCursor:null}}}}]);
}

function walk(x, rootDramaId="") {
  if (!x || typeof x !== "object") return;

  const dramaId = rootDramaId || (x.drama && x.drama.id) || "";

  if (Object.prototype.hasOwnProperty.call(x,"secretUnlocked")) x.secretUnlocked = true;
  if (Object.prototype.hasOwnProperty.call(x,"ageConfirmed")) x.ageConfirmed = true;
  ...
  if (!Array.isArray(x) && typeof x.id === "string") {
    ...
    if (episode) {
      ...
      if ("hlsUrl" in x) {
        if (CREATOR_MEDIA[dramaId]) x.hlsUrl = media(x.id, dramaId);
        else if (!x.hlsUrl) x.hlsUrl = media(x.id, dramaId);
      }
    }
  }
  Object.keys(x).forEach(k => walk(x[k], dramaId));
}
...
function flight(body) {
  ...
  body = body.replace(/\\"isFree\\":false/g,'\\"isFree\\":true')
    ...
    .replace(/\\"secretUnlocked\\":false/g,'\\"secretUnlocked\\":true')
    .replace(/\\"ageConfirmed\\":false/g,'\\"ageConfirmed\\":true')
    .replace(/\\"requiresAdultConfirm\\":true/g,'\\"requiresAdultConfirm\\":false');
  if (m) body = body.replace(/\\"hlsUrl\\":null/,
    `\\"hlsUrl\\":\\"${media(m[1])}\\"`);
  return body;
}

function rewrite(body, url="") {
  if (/secret\.list/.test(url)) return secretList();
  if (!body) return body;
...
if (typeof $done !== "undefined") {
  ...
  const body = rewrite($response.body, url);
  if (/secret\.list/.test(url)) {
